...
class PlayerProfile < ApplicationRecord
  ...
  def self.getSimilarProfiles(profile)
    ...
    if profile.game_ids.length > 0
      list = []
      profile.game_ids.each { |game| list << "game_id = #{game}" }
      ...
      .order('common_games DESC')
    end
    players
  end

    def getSpairingMatchRateStaticalWeigh(player)
      weighFunctions = [10, 10, 100]
    ...
    def getSpairingMatchRate(player, weighFunctions)
      ...
      weightedSPMatch += weighFunctions[1]*getSpairingWordCommentAffinity(player)
      weightedSPMatch += weighFunctions[2]*getSpairingTagAffinity(player)
